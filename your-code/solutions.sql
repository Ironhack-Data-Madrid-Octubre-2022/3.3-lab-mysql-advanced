SELECT  au_id 'AUTHOR ID', advance + sum(t.price * s.qty * t.royalty / 100 * ta.royaltyper / 100) 'SALES_ROYALTY'
FROM titleauthor ta
LEFT JOIN titles t
...
GROUP BY au_id, ta.title_id
ORDER BY advance + sum(t.price * s.qty * t.royalty / 100 * ta.royaltyper / 100) DESC LIMIT 3
;
-- CON SUBQUERY
...
-- CREANDO LA TABLA 
CREATE TABLE most_profiting_authors_1
SELECT  au_id 'AUTHOR ID', advance + sum(t.price * s.qty * t.royalty / 100 * ta.royaltyper / 100) 'SALES_ROYALTY'
FROM titleauthor ta
LEFT JOIN titles t
ON t.title_id = ta.title_id
